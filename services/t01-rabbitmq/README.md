...
Follow the steps below to configure RabbitMQ for the `t01-beamline` namespace.
...
```bash
$ # module load k8s-t01
$ head -c 500 /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1 | kubeseal --raw --namespace t01-beamline --name rabbitmq-secrets
```
...
`t01-rabbitmq-daq.diamond.ac.uk -> Static IP` this mapping allows processes outside of the namespace e.g. Analysis to access data from rabbitmq.
